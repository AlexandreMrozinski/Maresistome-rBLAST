...
# Calculer les moyennes et les écarts types pour all_filtered_dataVerveur
...
sd_length_verveur <- sd(all_filtered_dataVerveur$length, na.rm = TRUE)
sd_qcovs_verveur <- sd(all_filtered_dataVerveur$qcovs, na.rm = TRUE)
sd_pident_verveur <- sd(all_filtered_dataVerveur$pident, na.rm = TRUE)

# Afficher les moyennes et les écarts types pour all_filtered_dataVerveur
cat("Moyennes et écarts types pour all_filtered_dataVerveur:\n")
cat("length: ", mean_length_verveur, " (", sd_length_verveur, ")\n")
cat("qcovs: ", mean_qcovs_verveur, " (", sd_qcovs_verveur, ")\n")
cat("pident: ", mean_pident_verveur, " (", sd_pident_verveur, ")\n\n")

# Calculer les moyennes et les écarts types pour all_filtered_dataBarnenez
...
sd_length_barnenez <- sd(all_filtered_dataBarnenez$length, na.rm = TRUE)
sd_qcovs_barnenez <- sd(all_filtered_dataBarnenez$qcovs, na.rm = TRUE)
sd_pident_barnenez <- sd(all_filtered_dataBarnenez$pident, na.rm = TRUE)

# Afficher les moyennes et les écarts types pour all_filtered_dataBarnenez
cat("Moyennes et écarts types pour all_filtered_dataBarnenez:\n")
cat("length: ", mean_length_barnenez, " (", sd_length_barnenez, ")\n")
cat("qcovs: ", mean_qcovs_barnenez, " (", sd_qcovs_barnenez, ")\n")
cat("pident: ", mean_pident_barnenez, " (", sd_pident_barnenez, ")\n\n")

# Combiner les deux dataframes
combined_data <- rbind(all_filtered_dataVerveur, all_filtered_dataBarnenez)

# Calculer les moyennes et les écarts types pour le dataframe combiné
...
sd_length_combined <- sd(combined_data$length, na.rm = TRUE)
sd_qcovs_combined <- sd(combined_data$qcovs, na.rm = TRUE)
sd_pident_combined <- sd(combined_data$pident, na.rm = TRUE)

# Afficher les moyennes et les écarts types pour le dataframe combiné
cat("Moyennes et écarts types pour les deux sites combinés:\n")
cat("Length: ", mean_length_combined, " (", sd_length_combined, ")\n")
cat("Qcovs: ", mean_qcovs_combined, " (", sd_qcovs_combined, ")\n")
cat("Pident: ", mean_pident_combined, " (", sd_pident_combined, ")\n")
...
arbitrary_order <- c("Gammaproteobacteria", "Flavobacteriia", "Bacilli", "Cytophagia", "Actinomycetes", "Alphaproteobacteria", "Betaproteobacteria",  "Sphingobacteriia")  # Remplacer par l'ordre désiré
...
heatmap_plot_class <- ggplot(heatmap_data_class, aes(x = class, y = Condition, fill = count)) +
  ...
  labs(title = "Heatmap de présence des classes bactériens - Verveur et Barnenez", 
       x = "Classes bactériennes", 
       y = "Compartiments",
       fill = "Nombre 
d'isolats :") +  # Changer la légende
  annotate("text", x = 6, y = 8.7, size = 6,label = "n=6 (4%)", color = "black") +
  annotate("text", x = 7, y = 8.7, size = 6,label = "n=1 (<1%)", color = "black") +
  annotate("text", x = 1, y = 8.7, size = 6,label = "n=81 (55%)", color = "black") +
  annotate("text", x = 4, y = 8.7, size = 6,label = "n=9 (6%)", color = "black") +
  annotate("text", x = 2, y = 8.7, size = 6,label = "n=19 (13%)", color = "black") +
  annotate("text", x = 5, y = 8.7, size = 6,label = "n=9 (6%)", color = "black") +
  annotate("text", x = 3, y = 8.7, size = 6,label = "n=19 (13%)", color = "black") +
...
bar_plot_combinedgenresclasses <- ggplot(top_genres_combined3, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  ...
  labs(title = "Genres retrouvés sur les sites de Verveur et Barnenez", x = NULL, y = "Nombre d'isolats") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  scale_fill_manual(values = c("Barnenez" = "#145C9E", "Verveur" = "#59C9A5")) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 15, 1), limits = c(0, 16)) +
  annotate("text", x = 1, y = 16, size = 4, label = "9.65%", color = "black") +
  annotate("text", x = 2, y = 13, size = 4, label = "12.4%", color = "black") +
  annotate("text", x = 3, y = 10, size = 4, label = "5.5%", color = "black") +
  annotate("text", x = 4, y = 10, size = 4, label = "9%", color = "black") +
  annotate("text", x = 5, y = 8, size = 4, label = "6.2%", color = "black") +
  annotate("text", x = 6, y = 6, size = 4, label = "2.75%", color = "black") +
  annotate("text", x = 7, y = 6, size = 4, label = "5.5%", color = "black") +
  annotate("text", x = 8, y = 7, size = 4, label = "5.5%", color = "black") +
  annotate("text", x = 9, y = 6, size = 4, label = "4.1%", color = "black") +
  annotate("text", x = 10, y = 5, size = 4, label = "4.1%", color = "black") +
  annotate("text", x = 11, y = 5, size = 4, label = "2%", color = "black") +
  annotate("text", x = 12, y = 5, size = 4, label = "2%", color = "black") +
  annotate("text", x = 13, y = 6, size = 4, label = "3.4%", color = "black") +
  annotate("text", x = 14, y = 4, size = 4, label = "2.75%", color = "black") +
  annotate("text", x = 15, y = 4, size = 4, label = "1.4%", color = "black") +
  annotate("text", x = 16, y = 4, size = 4, label = "1.4%", color = "black") +
  annotate("text", x = 17, y = 4, size = 4, label = "1.4%", color = "black") +
  annotate("text", x = 18, y = 4, size = 4, label = "2%", color = "black") +
  #bracket
  ...
  annotate("text", x = 31, y = 3.2, size = 4, label = "18.6%", color = "black") + #texte au dessus du bracket
  theme(legend.position = "right", 
...
#Légende symboles pour classes bactériennes 
legend_text <- paste(names(class_symbols), class_symbols, sep = ": ")
legend_text <- paste(legend_text, collapse = "\n")
legend_title <- "Classes bactériennes :"
# Concaténer les textes avec le titre de la légende
legend_text <- paste(legend_title, "\n", legend_text)
bar_plot_combinedgenresclasses <- bar_plot_combinedgenresclasses + 
  annotate("text", x = Inf, y = Inf, label = legend_text, hjust = 1, vjust = 1, size = 7, color = "black", parse = FALSE)
...
d'isolats :") + # Modifier le titre de la légende
  labs(x = "Genres bacteriens", y = "Compartiments") +
