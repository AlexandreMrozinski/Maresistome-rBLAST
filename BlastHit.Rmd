...
total_genres <- length(unique(all_filtered_dataMorlaix$genus))

# Créez le graphique à barres
bar_plotMorlaixTot <- ggplot(data.frame(Genus = names(genre_countsMorlaix), Count = as.numeric(genre_countsMorlaix)), 
                   aes(x = reorder(Genus, -Count), y = Count)) +
  ...
  labs(title = "Genres retrouvés pour Morlaix : Eau / Sédiments / GD (DEP et ND)", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  annotate("text", x = 30, y = 12, label = paste("Total de séquences :", nrow(all_filtered_dataMorlaix)), hjust = 1, vjust = 1) +
...
bar_plot_combined2 <- ggplot(top_genres_combined2, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("Morlaix" = "#145C9E", "Verveur" = "#59C9A5")) +
...
```{r,}
# Création des dataframes avec une colonne "Count" pour Morlaix
genre_countsMorlaix <- table(all_filtered_dataMorlaix$genus)
total_sequencesMorlaix <- sum(genre_countsMorlaix)
total_genresMorlaix <- length(unique(all_filtered_dataMorlaix$genus))
top_genresMorlaix_df <- data.frame(Genus = names(genre_countsMorlaix), Count = as.numeric(genre_countsMorlaix))
top_genresMorlaix_df$Ville <- "Morlaix"
...
# Combinaison des deux dataframes
top_genres_combined3 <- rbind(top_genresMorlaix_df, top_genresVerveur_df)

# Nombre total de genres pour les deux sites combinés
total_genres_combined3 <- length(unique(c(all_filtered_dataMorlaix$genus, all_filtered_dataVerveur$genus)))
...
bar_plot_combined3 <- ggplot(top_genres_combined3, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  ...
  scale_fill_manual(values = c("Morlaix" = "#145C9E", "Verveur" = "#59C9A5")) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 15, 1), limits = c(0, 15)) +  # Modifier l'échelle des données y
  annotate("text", x = Inf, y = 9, label = paste("Total de séquences pour Morlaix:", total_sequencesMorlaix, "\nNombre de genres pour Morlaix:", total_genresMorlaix, "\nTotal de séquences pour Verveur:", total_sequencesVerveur, "\nNombre de genres pour Verveur:", total_genresVerveur, "\nNombre total de genres pour les deux sites combinés:", total_genres_combined3), hjust = 1, vjust = -0.1)

print(bar_plot_combined3)
```

```{r}
# Filtrer les données pour chaque échantillon
ME_data <- all_filtered_dataMorlaix[grep("^ME", all_filtered_dataMorlaix$qseqid), ]
MS_data <- all_filtered_dataMorlaix[grep("^MS", all_filtered_dataMorlaix$qseqid), ]
MG_data <- all_filtered_dataMorlaix[grep("^MG", all_filtered_dataMorlaix$qseqid), ]

VE_data <- all_filtered_dataVerveur[grep("^VE", all_filtered_dataVerveur$qseqid), ]
VS_data <- all_filtered_dataVerveur[grep("^VS", all_filtered_dataVerveur$qseqid), ]
VG_data <- all_filtered_dataVerveur[grep("^VG", all_filtered_dataVerveur$qseqid), ]

# Compter le nombre d'occurrences de chaque genre pour chaque échantillon
genre_counts_ME <- table(ME_data$genus)
genre_counts_MS <- table(MS_data$genus)
genre_counts_MG <- table(MG_data$genus)

genre_counts_VE <- table(VE_data$genus)
genre_counts_VS <- table(VS_data$genus)
genre_counts_VG <- table(VG_data$genus)

# Créer une dataframe pour chaque échantillon
heatmap_data_ME <- data.frame(genus = names(genre_counts_ME), count = as.numeric(genre_counts_ME), Condition = "MorlaixEau")
heatmap_data_MS <- data.frame(genus = names(genre_counts_MS), count = as.numeric(genre_counts_MS), Condition = "MorlaixSedi")
heatmap_data_MG <- data.frame(genus = names(genre_counts_MG), count = as.numeric(genre_counts_MG), Condition = "MorlaixGD")

heatmap_data_VE <- data.frame(genus = names(genre_counts_VE), count = as.numeric(genre_counts_VE), Condition = "VerveurEau")
heatmap_data_VS <- data.frame(genus = names(genre_counts_VS), count = as.numeric(genre_counts_VS), Condition = "VerveurSedi")
heatmap_data_VG <- data.frame(genus = names(genre_counts_VG), count = as.numeric(genre_counts_VG), Condition = "VerveurGD")

# Fusionner les dataframes
heatmap_data <- rbind(heatmap_data_ME, heatmap_data_MS, heatmap_data_MG, heatmap_data_VE, heatmap_data_VS, heatmap_data_VG)

# heatmap
heatmap_plot <- ggplot(heatmap_data, aes(x = genus, y = Condition, fill = count)) +
  geom_tile() +
  geom_text(aes(label = count), vjust = 0.5, color = "white") +  # Ajouter le nombre de genre
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(title = "Heatmap de présence des genres bactériens - Verveur et Morlaix", x = "Genre", y = "Conditions") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(hjust = 0.5))
print(heatmap_plot)
```
```{r}
total_counts <- heatmap_data %>%
  group_by(genus) %>%
  summarise(total_count = sum(count))

# Afficher le nombre total d'apparitions pour chaque genre
print(total_counts)
```
...
# Compter le nombre d'occurrences de chaque genre pour chaque échantillon
genre_counts_ME <- table(ME_data$class)
genre_counts_MS <- table(MS_data$class)
genre_counts_MG <- table(MG_data$class)

genre_counts_VE <- table(VE_data$class)
genre_counts_VS <- table(VS_data$class)
genre_counts_VG <- table(VG_data$class)

# Créer une dataframe pour chaque échantillon
heatmap_data_ME <- data.frame(class = names(genre_counts_ME), count = as.numeric(genre_counts_ME), Condition = "MorlaixEau")
heatmap_data_MS <- data.frame(class = names(genre_counts_MS), count = as.numeric(genre_counts_MS), Condition = "MorlaixSedi")
heatmap_data_MG <- data.frame(class = names(genre_counts_MG), count = as.numeric(genre_counts_MG), Condition = "MorlaixGD")

heatmap_data_VE <- data.frame(class = names(genre_counts_VE), count = as.numeric(genre_counts_VE), Condition = "VerveurEau")
heatmap_data_VS <- data.frame(class = names(genre_counts_VS), count = as.numeric(genre_counts_VS), Condition = "VerveurSedi")
heatmap_data_VG <- data.frame(class = names(genre_counts_VG), count = as.numeric(genre_counts_VG), Condition = "VerveurGD")

# Fusionner les dataframes
heatmap_data <- rbind(heatmap_data_ME, heatmap_data_MS, heatmap_data_MG, heatmap_data_VE, heatmap_data_VS, heatmap_data_VG)

# heatmap
heatmap_plotclass <- ggplot(heatmap_data, aes(x = class, y = Condition, fill = count)) +
  geom_tile() +
  geom_text(aes(label = count), vjust = 0.5, color = "white") +  # Ajouter le nombre de phylum
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(title = "Heatmap de présence des classes bactériennes - Verveur et Morlaix", x = "classes", y = "Conditions") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(hjust = 0.5))
print(heatmap_plotclass)
```
...
bar_plot_MorlaixEau <- ggplot(data.frame(Genus = names(genre_counts_MorlaixEau), Count = as.numeric(genre_counts_MorlaixEau)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Morlaix Eau", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_MorlaixSedi <- ggplot(data.frame(Genus = names(genre_counts_MorlaixSedi), Count = as.numeric(genre_counts_MorlaixSedi)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Morlaix Sédiments", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  # Ajout de l'annotation pour le nombre total de séquences
...
bar_plot_MorlaixGDND <- ggplot(data.frame(Genus = names(genre_counts_MorlaixGDND), Count = as.numeric(genre_counts_MorlaixGDND)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 3, 1), limits = c(0, 3)) +  # Modifier l'échelle des données y
  labs(title = "Genres retrouvés - Morlaix GDND", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_MorlaixGDDEP <- ggplot(data.frame(Genus = names(genre_counts_MorlaixGDDEP), Count = as.numeric(genre_counts_MorlaixGDDEP)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Morlaix GDDEP", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
grid.arrange(heatmap_plotclass)
...
grid.arrange(bar_plotMorlaixTot)

grid.arrange(bar_plotVerveurTot)


grid.arrange(bar_plotMorlaix, bar_plotVerveur,
             ncol = 2)
...
ggsave("Figures/FiguresCommun/heatmap_plot.png", heatmap_plot, width = 10, height = 6, units = "in")
ggsave("Figures/FiguresCommun/heatmap_plotclass.png", heatmap_plotclass, width = 10, height = 6, units = "in")
...
ggsave("Figures/FiguresMorlaix/venn_plotMorlaix.png", venn_plotMorlaix, width = 6, height = 6, units = "in")
ggsave("Figures/FiguresMorlaix/bar_plotMorlaixTot.png", bar_plotMorlaixTot, width = 6, height = 6, units = "in")
