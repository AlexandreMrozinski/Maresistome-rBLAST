...
genre_countsBarnenez <- table(all_filtered_dataBarnenez$genus)

# Sélectionnez les 10 genres les plus retrouvés
top_genresBarnenez <- head(sort(genre_countsBarnenez, decreasing = TRUE), 10)
...
bar_plotBarnenez <- ggplot(data.frame(Genus = names(top_genresBarnenez), Count = as.numeric(top_genresBarnenez)), 
  ...
  labs(title = "Top 10 Genres les plus retrouvés pour Barnenez : Eau / Sédiments / GD (DEP et ND)", x = "Genre", y = "Nombre de séquences") +
...
# Créez le graphique à barres
bar_plotBarnenezTot <- ggplot(data.frame(Genus = names(genre_countsBarnenez), Count = as.numeric(genre_countsBarnenez)), 
                   aes(x = reorder(Genus, -Count), y = Count)) +
  ...
  labs(title = "Genres retrouvés pour Barnenez : Eau / Sédiments / GD (DEP et ND)", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
# Créez le graphique à barres
bar_plotVerveurTot <- ggplot(data.frame(Genus = names(genre_countsVerveur), Count = as.numeric(genre_countsVerveur)), 
                   aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), vjust = -0.5, color = "black", size = 3.5) +  # Ajouter les valeurs au-dessus de chaque barre
  labs(title = "Genres retrouvés pour Verveur : Eau / Sédiments / GD (DEP et ND)", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
genre_countsBarnenez <- table(all_filtered_dataBarnenez$genus)
total_sequencesBarnenez <- sum(genre_countsBarnenez)
total_genresBarnenez <- length(unique(all_filtered_dataBarnenez$genus))
top_genresBarnenez_df <- data.frame(Genus = names(genre_countsBarnenez), Count = as.numeric(genre_countsBarnenez))
top_genresBarnenez_df$Ville <- "Barnenez"

# Création des dataframes avec une colonne "Count" pour Verveur
genre_countsVerveur <- table(all_filtered_dataVerveur$genus)
total_sequencesVerveur <- sum(genre_countsVerveur)
total_genresVerveur <- length(unique(all_filtered_dataVerveur$genus))
top_genresVerveur_df <- data.frame(Genus = names(genre_countsVerveur), Count = as.numeric(genre_countsVerveur))
top_genresVerveur_df$Ville <- "Verveur"
...
bar_plot_combined3 <- ggplot(top_genres_combined3, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +  # Ajouter les valeurs au-dessus de chaque barre
  labs(title = "Genres les plus retrouvés pour les deux sites", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("Barnenez" = "#145C9E", "Verveur" = "#59C9A5")) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 15, 1), limits = c(0, 15)) +  # Modifier l'échelle des données y
...
```{r}
# Compter le nombre d'occurrences de chaque genre pour chaque échantillon
genre_counts_BarnenezGDND <- table(filtered_data_BarnenezGDND$genus)
genre_counts_BarnenezGDDEP <- table(filtered_data_BarnenezGDDEP$genus)
genre_counts_BarnenezEau <- table(filtered_data_BarnenezEau$genus)
genre_counts_BarnenezSedi <- table(filtered_data_BarnenezSedi$genus)
...
heatmap_datagenus_BarnenezGDND <- data.frame(genus = names(genre_counts_BarnenezGDND), count = as.numeric(genre_counts_BarnenezGDND), Condition = "BarnenezGDND")
heatmap_datagenus_BarnenezGDDEP <- data.frame(genus = names(genre_counts_BarnenezGDDEP), count = as.numeric(genre_counts_BarnenezGDDEP), Condition = "BarnenezGDDEP")
heatmap_datagenus_BarnenezEau <- data.frame(genus = names(genre_counts_BarnenezEau), count = as.numeric(genre_counts_BarnenezEau), Condition = "BarnenezEau")
heatmap_datagenus_BarnenezSedi <- data.frame(genus = names(genre_counts_BarnenezSedi), count = as.numeric(genre_counts_BarnenezSedi), Condition = "BarnenezSedi")

heatmap_datagenus_VerveurGDND <- data.frame(genus = names(genre_counts_VerveurGDND), count = as.numeric(genre_counts_VerveurGDND), Condition = "VerveurGDND")
heatmap_datagenus_VerveurGDDEP <- data.frame(genus = names(genre_counts_VerveurGDDEP), count = as.numeric(genre_counts_VerveurGDDEP), Condition = "VerveurGDDEP")
heatmap_datagenus_VerveurEau <- data.frame(genus = names(genre_counts_VerveurEau), count = as.numeric(genre_counts_VerveurEau), Condition = "VerveurEau")
heatmap_datagenus_VerveurSedi <- data.frame(genus = names(genre_counts_VerveurSedi), count = as.numeric(genre_counts_VerveurSedi), Condition = "VerveurSedi")
...
heatmap_plot_genus <- ggplot(heatmap_data_genus, aes(x = genus, y = Condition, fill = count)) +
  ...
  labs(title = "Heatmap de présence des genres bactériens - Verveur et Barnenez", x = "Genre", y = "Conditions") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
```{r}
# Compter le nombre d'occurrences de chaque genre pour chaque échantillon
genre_counts_BarnenezGDND <- table(filtered_data_BarnenezGDND$class)
genre_counts_BarnenezGDDEP <- table(filtered_data_BarnenezGDDEP$class)
genre_counts_BarnenezEau <- table(filtered_data_BarnenezEau$class)
genre_counts_BarnenezSedi <- table(filtered_data_BarnenezSedi$class)

genre_counts_VerveurGDND <- table(filtered_data_VerveurGDND$class)
genre_counts_VerveurGDDEP <- table(filtered_data_VerveurGDDEP$class)
genre_counts_VerveurEau <- table(filtered_data_VerveurEau$class)
genre_counts_VerveurSedi <- table(filtered_data_VerveurSedi$class)

# Créer une dataframe pour chaque échantillon
heatmap_dataclass_BarnenezGDND <- data.frame(class = names(genre_counts_BarnenezGDND), count = as.numeric(genre_counts_BarnenezGDND), Condition = "BarnenezGDND")
heatmap_dataclass_BarnenezGDDEP <- data.frame(class = names(genre_counts_BarnenezGDDEP), count = as.numeric(genre_counts_BarnenezGDDEP), Condition = "BarnenezGDDEP")
heatmap_dataclass_BarnenezEau <- data.frame(class = names(genre_counts_BarnenezEau), count = as.numeric(genre_counts_BarnenezEau), Condition = "BarnenezEau")
heatmap_dataclass_BarnenezSedi <- data.frame(class = names(genre_counts_BarnenezSedi), count = as.numeric(genre_counts_BarnenezSedi), Condition = "BarnenezSedi")

heatmap_dataclass_VerveurGDND <- data.frame(class = names(genre_counts_VerveurGDND), count = as.numeric(genre_counts_VerveurGDND), Condition = "VerveurGDND")
heatmap_dataclass_VerveurGDDEP <- data.frame(class = names(genre_counts_VerveurGDDEP), count = as.numeric(genre_counts_VerveurGDDEP), Condition = "VerveurGDDEP")
heatmap_dataclass_VerveurEau <- data.frame(class = names(genre_counts_VerveurEau), count = as.numeric(genre_counts_VerveurEau), Condition = "VerveurEau")
heatmap_dataclass_VerveurSedi <- data.frame(class = names(genre_counts_VerveurSedi), count = as.numeric(genre_counts_VerveurSedi), Condition = "VerveurSedi")
...
heatmap_plot_class <- ggplot(heatmap_data_class, aes(x = class, y = Condition, fill = count)) +
  ...
  geom_text(aes(label = count), vjust = 0.5, color = "white") +  
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(title = "Heatmap de présence des classes bactériens - Verveur et Barnenez", x = "Classes", y = "Conditions") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(hjust = 0.5))
...
bar_plot_BarnenezEau <- ggplot(data.frame(Genus = names(genre_counts_BarnenezEau), Count = as.numeric(genre_counts_BarnenezEau)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Barnenez Eau", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_VerveurEau <- ggplot(data.frame(Genus = names(genre_counts_VerveurEau), Count = as.numeric(genre_counts_VerveurEau)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Verveur Eau", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  # Ajout de l'annotation pour le nombre total de séquences
...
bar_plot_BarnenezSedi <- ggplot(data.frame(Genus = names(genre_counts_BarnenezSedi), Count = as.numeric(genre_counts_BarnenezSedi)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Barnenez Sédiments", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  # Ajout de l'annotation pour le nombre total de séquences
...
bar_plot_VerveurSedi <- ggplot(data.frame(Genus = names(genre_counts_VerveurSedi), Count = as.numeric(genre_counts_VerveurSedi)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Verveur Sédiments", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  # Ajout de l'annotation pour le nombre total de séquences
...
bar_plot_BarnenezGDND <- ggplot(data.frame(Genus = names(genre_counts_BarnenezGDND), Count = as.numeric(genre_counts_BarnenezGDND)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 3, 1), limits = c(0, 3)) +  # Modifier l'échelle des données y
  labs(title = "Genres retrouvés - Barnenez GDND", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_VerveurGDND <- ggplot(data.frame(Genus = names(genre_counts_VerveurGDND), Count = as.numeric(genre_counts_VerveurGDND)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Verveur GDND", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_BarnenezGDDEP <- ggplot(data.frame(Genus = names(genre_counts_BarnenezGDDEP), Count = as.numeric(genre_counts_BarnenezGDDEP)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Barnenez GDDEP", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_VerveurGDDEP <- ggplot(data.frame(Genus = names(genre_counts_VerveurGDDEP), Count = as.numeric(genre_counts_VerveurGDDEP)), 
                              aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "Genres retrouvés - Verveur GDDEP", x = "Genre", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
venn_plotBarnenez <- ggvenn(sets_listBarnenez, show_elements = TRUE,
  ...
  digits = 1,
  fill_color = c("#D7263D", "#F46036", "#1B998B", "#C5D86D"),
  fill_alpha = 1.5,
  stroke_color = "black",
  stroke_alpha = 1,
  stroke_size = 1,
  ...
  set_name_size = 3,  # Ajustez la taille de la police ici BarnenezSedi BarnenezEau BarnenezGD..
...
ggsave("Figures/FiguresBarnenez/bar_plot_BarnenezGDDEP.png", bar_plot_BarnenezGDDEP, width = 6, height = 4, units = "in")
...
ggsave("Figures/FiguresVerveur/venn_plotVerveur.png", venn_plotVerveur, width = 6, height = 6, units = "in")
