...
cltax_BarnenezSedi=cbind(cl_BarnenezSedi,taxlist_BarnenezSedi) #bind BLAST hits and taxonomy table
...
```{r, Représentation graphique des classs les plus retrouvés pour Barnenez Eau / Sédiments / GD}
...
# Tableau de comptage des classs
class_countsBarnenez <- table(all_filtered_dataBarnenez$genus)

# Sélectionnez les 10 classs les plus retrouvés
top_classsBarnenez <- head(sort(class_countsBarnenez, decreasing = TRUE), 10)

# Créez le graphique à barres
bar_plotBarnenez <- ggplot(data.frame(Genus = names(top_classsBarnenez), Count = as.numeric(top_classsBarnenez)), 
                   aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  labs(title = "Top 10 classs les plus retrouvés pour Barnenez : Eau / Sédiments / GD (DEP et ND)", x = "class", y = "Nombre de séquences") +
...
# Compter le nombre total de classs
total_classs <- length(unique(all_filtered_dataBarnenez$genus))

# Créez le graphique à barres
bar_plotBarnenezTot <- ggplot(data.frame(Genus = names(class_countsBarnenez), Count = as.numeric(class_countsBarnenez)), 
                   aes(x = reorder(Genus, -Count), y = Count)) +
  ...
  labs(title = "classs retrouvés pour Barnenez : Eau / Sédiments / GD (DEP et ND)", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  annotate("text", x = 30, y = 12, label = paste("Total de séquences :", nrow(all_filtered_dataBarnenez)), hjust = 1, vjust = 1) +
  annotate("text", x = 30, y = Inf, label = paste("Nombre total de classs :", total_classs), hjust = 1, vjust = 0.95) # Ajouter le nombre total de classs

print(bar_plotBarnenezTot)
```


```{r, Représentation graphique des classs les plus retrouvés pour Verveur Eau / Sédiments / GD}
# Filtrer pour ne conserver que le résultat avec le plus grand pident par fichier .fa
...
# Tableau de comptage des classs
class_countsVerveur <- table(all_filtered_dataVerveur$genus)

# Sélectionnez les 10 classs les plus retrouvés
top_classsVerveur <- head(sort(class_countsVerveur, decreasing = TRUE), 10)

# Créez le graphique à barres
bar_plotVerveur <- ggplot(data.frame(Genus = names(top_classsVerveur), Count = as.numeric(top_classsVerveur)), 
                   aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), vjust = -0.5, color = "black", size = 3.5) +  # Ajouter les valeurs au-dessus de chaque barre
  labs(title = "Top 10 classs les plus retrouvés pour Verveur : Eau / Sédiments / GD (DEP et ND)", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
# Compter le nombre total de classs
total_classs <- length(unique(all_filtered_dataVerveur$genus))

# Créez le graphique à barres
bar_plotVerveurTot <- ggplot(data.frame(Genus = names(class_countsVerveur), Count = as.numeric(class_countsVerveur)), 
                   aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), vjust = -0.5, color = "black", size = 3.5) +  # Ajouter les valeurs au-dessus de chaque barre
  labs(title = "classs retrouvés pour Verveur : Eau / Sédiments / GD (DEP et ND)", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  annotate("text", x = 27, y = 12, label = paste("Total de séquences :", nrow(all_filtered_dataVerveur)), hjust = 1, vjust = 1) +
  annotate("text", x = 27, y = Inf, label = paste("Nombre total de classs :", total_classs), hjust = 1, vjust = 0.95) # Ajouter le nombre total de classs
...
top_classs_combined <- rbind(
  data.frame(Genus = names(top_classsBarnenez), Count = as.numeric(top_classsBarnenez)),
  data.frame(Genus = names(top_classsVerveur), Count = as.numeric(top_classsVerveur))
)

# Agrégation des comptes pour chaque class
top_classs_combined <- aggregate(Count ~ Genus, data = top_classs_combined, sum)

# Tracé du graphique
bar_plot_combined <- ggplot(top_classs_combined, aes(x = reorder(Genus, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), vjust = -0.5, color = "black", size = 3.5) +  # Ajouter les valeurs au-dessus de chaque barre
  labs(title = "Top 10 classs les plus retrouvés pour les deux sites combinés", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 20, 1), limits = c(0, 20)) +  # Modifier l'échelle des données y
  annotate("text", x = Inf, y = -Inf, label = paste("Total de séquences :", sum(top_classs_combined$Count)), hjust = 1, vjust = 0)

print(bar_plot_combined)
```

```{r, Représentation graphique des classs les plus retrouvés pour Verveur et Barnenez Eau / Sédiments / GD}
# Création des dataframes avec une colonne "Count"
top_classsBarnenez_df <- data.frame(Genus = names(top_classsBarnenez), Count = as.numeric(top_classsBarnenez))
top_classsVerveur_df <- data.frame(Genus = names(top_classsVerveur), Count = as.numeric(top_classsVerveur))

# Ajout d'une colonne "Ville" pour chaque dataframe
top_classsBarnenez_df$Ville <- "Barnenez"
top_classsVerveur_df$Ville <- "Verveur"

# Combinaison des deux dataframes
top_classs_combined2 <- rbind(top_classsBarnenez_df, top_classsVerveur_df)

# Tracé du graphique combiné
bar_plot_combined2 <- ggplot(top_classs_combined2, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  ...
  labs(title = "Top 10 classs les plus retrouvés pour les deux sites", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ...
  annotate("text", x = Inf, y = 12, label = paste("Total de séquences pour Barnenez:", sum(top_classsBarnenez), "\nTotal de séquences pour Verveur:", sum(top_classsVerveur)), hjust = 1, vjust = -0.1)
...
class_countsBarnenez <- table(all_filtered_dataBarnenez$genus)
total_sequencesBarnenez <- sum(class_countsBarnenez)
total_classsBarnenez <- length(unique(all_filtered_dataBarnenez$genus))
top_classsBarnenez_df <- data.frame(Genus = names(class_countsBarnenez), Count = as.numeric(class_countsBarnenez))
top_classsBarnenez_df$Ville <- "Barnenez"

# Création des dataframes avec une colonne "Count" pour Verveur
class_countsVerveur <- table(all_filtered_dataVerveur$genus)
total_sequencesVerveur <- sum(class_countsVerveur)
total_classsVerveur <- length(unique(all_filtered_dataVerveur$genus))
top_classsVerveur_df <- data.frame(Genus = names(class_countsVerveur), Count = as.numeric(class_countsVerveur))
top_classsVerveur_df$Ville <- "Verveur"

# Combinaison des deux dataframes
top_classs_combined3 <- rbind(top_classsBarnenez_df, top_classsVerveur_df)

# Nombre total de classs pour les deux sites combinés
total_classs_combined3 <- length(unique(c(all_filtered_dataBarnenez$genus, all_filtered_dataVerveur$genus)))

# Tracé du graphique combiné
bar_plot_combined3 <- ggplot(top_classs_combined3, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +  # Ajouter les valeurs au-dessus de chaque barre
  labs(title = "classs les plus retrouvés pour les deux sites", x = "classs", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("Barnenez" = "#145C9E", "Verveur" = "#59C9A5")) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 15, 1), limits = c(0, 15)) +  # Modifier l'échelle des données y
  annotate("text", x = Inf, y = 9, label = paste("Total de séquences pour Barnenez:", total_sequencesBarnenez, "\nNombre de classs pour Barnenez:", total_classsBarnenez, "\nTotal de séquences pour Verveur:", total_sequencesVerveur, "\nNombre de classs pour Verveur:", total_classsVerveur, "\nNombre total de classs pour les deux sites combinés:", total_classs_combined3), hjust = 1, vjust = -0.1)
...
class_symbols <- c("Bacilli" = "★", "Actinomycetes" = "○", "Alphaproteobacteria" = "□", 
                   "Betaproteobacteria" = "◇", "Cytophagia" = "△", "Flavobacteriia" = "▲", 
...
# Ajouter la colonne "Class" et "symbol" en fusionnant avec le data frame bacteria_db
top_classs_combined3 <- merge(top_classs_combined3, bacteria_db, by.x = "Genus", by.y = "genus", all.x = TRUE)

# Ajouter une colonne pour savoir si le genre a déjà été rencontré et calculer la position des labels
top_classs_combined3 <- top_classs_combined3[order(top_classs_combined3$Genus, top_classs_combined3$Ville), ]
top_classs_combined3$duplicated_genus <- duplicated(top_classs_combined3$Genus) | duplicated(top_classs_combined3$Genus, fromLast = TRUE)

# Ajouter une colonne max_count pour la hauteur maximale de la barre pour chaque genre
top_classs_combined3 <- top_classs_combined3 %>%
  group_by(Genus) %>%
  mutate(max_count = max(Count)) %>%
  ungroup()

# Déterminer la position des labels (centre ou côté)
top_classs_combined3$Label <- ifelse(top_classs_combined3$duplicated_genus, "", top_classs_combined3$symbol)
top_classs_combined3$symbol_y_pos <- ifelse(top_classs_combined3$duplicated_genus, top_classs_combined3$max_count, top_classs_combined3$Count)

# Nombre total de classes pour les deux sites combinés
total_classs_combined3 <- length(unique(c(all_filtered_dataBarnenez$genus, all_filtered_dataVerveur$genus)))

# Tracé du graphique combiné
bar_plot_combined3 <- ggplot(top_classs_combined3, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(data = subset(top_classs_combined3, duplicated_genus == FALSE), aes(label = Label), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 9) +  # Ajouter les symboles sur le côté
  geom_text(data = subset(top_classs_combined3, duplicated_genus == TRUE), aes(label = symbol, x = Genus, y = symbol_y_pos + 0.5), color = "black", size = 9) +  # Ajouter les symboles au centre au-dessus de la barre la plus haute
  labs(title = "Genres retrouvés pour le site de Verveur et Barnenez", x = "Genres", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12)) +
  scale_fill_manual(values = c("Barnenez" = "#145C9E", "Verveur" = "#59C9A5")) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 15, 1), limits = c(0, 15)) +  # Modifier l'échelle des données y
  annotate("text", x = 38, y = 12, label = paste("Total de séquences pour Barnenez:", total_sequencesBarnenez, "\nNombre de séquences pour Barnenez:", total_classsBarnenez, "\nTotal de séquences pour Verveur:", total_sequencesVerveur, "\nNombre de classes pour Verveur:", total_classsVerveur, "\nNombre total de genres pour les deux sites combinés:", total_classs_combined3), hjust = 1, vjust = -0.1, size = 5) +
...
bar_plot_combined3 <- bar_plot_combined3 + 
...
# Générer le barplot avec les genres triés dans l'ordre souhaité
bar_plot_combined3 <- ggplot(top_classs_combined3, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(data = subset(top_classs_combined3, duplicated_genus == FALSE), aes(label = Label), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 7) +
  geom_text(data = subset(top_classs_combined3, duplicated_genus == TRUE), aes(label = symbol, x = Genus, y = symbol_y_pos + 1), color = "black", size = 7) +
  labs(title = "Genres retrouvées sur les sites de Verveur et Barnenez", x = NULL, y = "Nombre de séquences") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  scale_fill_manual(values = c("Barnenez" = "#145C9E", "Verveur" = "#59C9A5")) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 15, 1), limits = c(0, 16)) +
  annotate("text", x = 38, y = 9.5, label = paste("Total de séquences pour Barnenez:", total_sequencesBarnenez, "\nNombre de séquences pour Barnenez:", total_classsBarnenez, "\nTotal de séquences pour Verveur:", total_sequencesVerveur, "\nNombre de classes pour Verveur:", total_classsVerveur, "\nNombre total de séquences pour les deux sites combinés:", total_classs_combined3), hjust = 1, vjust = -0.1, size = 5) +
  theme(legend.position = "right", 
        legend.text = element_text(size = 15), # Modifier la taille de la légende
        axis.title.y = element_text(size = 15), # Modifier la taille du titre de l'axe des ordonnées
        axis.text.y = element_text(size = 15)) + # Modifier la taille des chiffres de l'axe des ordonnées
  guides(fill = guide_legend(title = "Sites:", title.theme = element_text(size = 15))) # Modifier la taille du titre de la légende
...
de séquences :") + # Modifier le titre de la légende
  labs(x = "Genres", y = "Compartiments") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 15),
        axis.text.y = element_text(size = 12), 
        plot.title = element_text(hjust = 0.5),
        legend.text = element_text(size = 15), # Modifier la taille du texte de la légende
        legend.title = element_text(size = 15), # Modifier la taille du titre de la légende
        axis.title = element_text(size = 15)) # Modifier la taille du texte des étiquettes des axes x et y


# Combiner les deux graphiques en un seul plot verticalement aligné
combined_plot <- bar_plot_combined3 / heatmap_plot_genus + plot_layout(ncol = 1, heights = c(1, 2))

combined_plot <- wrap_plots(bar_plot_combined3, heatmap_plot_genus, ncol = 1)
print(combined_plot)
```
```{r}
ggsave("Figures/combined_plot.png", combined_plot, width = 20, height = 10, units = "in")
```
...
bar_plot_combined3 <- ggplot(top_classs_combined3, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(data = subset(top_classs_combined3, duplicated_genus == FALSE), aes(label = Label), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 9) +
  geom_text(data = subset(top_classs_combined3, duplicated_genus == TRUE), aes(label = symbol, x = Genus, y = symbol_y_pos + 0.5), color = "black", size = 9) +
  labs(title = "Classes les plus retrouvées pour les deux sites", x = "Genus", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("Barnenez" = "#145C9E", "Verveur" = "#59C9A5")) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 15, 1), limits = c(0, 15)) +
  annotate("text", x = Inf, y = 9, label = paste("Total de séquences pour Barnenez:", total_sequencesBarnenez, "\nNombre de séquences pour Barnenez:", total_classsBarnenez, "\nTotal de séquences pour Verveur:", total_sequencesVerveur, "\nNombre de classes pour Verveur:", total_classsVerveur, "\nNombre total de séquences pour les deux sites combinés:", total_classs_combined3), hjust = 1, vjust = -0.1) +
  theme(legend.position = "bottom") +
  guides(fill = guide_legend(title = "Ville"))

# Créer une légende manuelle pour les symboles
legend_text <- paste(names(class_symbols), class_symbols, sep = ": ")
legend_text <- paste(legend_text, collapse = "\n")
bar_plot_combined3 <- bar_plot_combined3 + 
  annotate("text", x = Inf, y = Inf, label = legend_text, hjust = 1, vjust = 1, size = 4, color = "black", parse = FALSE)

# Générer la heatmap avec les genres triés dans le même ordre que le barplot
heatmap_plot_genus <- ggplot(heatmap_data_genus, aes(x = reorder(genus, -count), y = Condition, fill = count)) +
  geom_tile() +
  geom_text(aes(label = count), vjust = 0.5, color = "white") +  
  scale_fill_gradient(low = "skyblue", high = "darkblue") +
  labs(title = "Heatmap de présence des genus bactériens - Verveur et Barnenez", x = "genus", y = "Conditions") +
...
bar_plot_combined3 <- ggplot(top_classs_combined3, aes(x = reorder(Genus, -Count), y = Count, fill = Ville)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Classes les plus retrouvées pour les deux sites", x = NULL, y = "Nombre de séquences") +  # Modifier le titre de l'axe des abscisses
  theme(axis.text.x = element_blank(),  # Supprimer le texte de l'axe des abscisses
        axis.ticks.x = element_blank()) +  # Supprimer les ticks de l'axe des abscisses
  scale_fill_manual(values = c("Barnenez" = "#145C9E", "Verveur" = "#59C9A5")) +
  scale_y_continuous(labels = scales::comma, breaks = seq(0, 15, 1), limits = c(0, 15)) +  
  annotate("text", x = Inf, y = 9, label = paste("Total de séquences pour Barnenez:", total_sequencesBarnenez, "\nNombre de séquences pour Barnenez:", total_classsBarnenez, "\nTotal de séquences pour Verveur:", total_sequencesVerveur, "\nNombre de classes pour Verveur:", total_classsVerveur, "\nNombre total de séquences pour les deux sites combinés:", total_classs_combined3), hjust = 1, vjust = -0.1) +
  theme(legend.position = "none")  # Supprimer la légende

# Ne pas ajouter les noms de genres
bar_plot_combined3 <- bar_plot_combined3 + 
  geom_text(data = NULL, aes(label = ""), position = "identity")

library(cowplot)

# Organiser les graphiques avec le barplot au-dessus
combined_plot <- plot_grid(
  bar_plot_combined3,
  heatmap_plot_genus,
  ncol = 1, align = 'v', axis = 'lr'  # Alignement vertical (par Condition)
)
...
bar_plot_BarnenezEau <- ggplot(data.frame(Genus = names(class_counts_BarnenezEau), Count = as.numeric(class_counts_BarnenezEau)), 
  ...
  labs(title = "classs retrouvés - Barnenez Eau", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_VerveurEau <- ggplot(data.frame(Genus = names(class_counts_VerveurEau), Count = as.numeric(class_counts_VerveurEau)), 
  ...
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "classs retrouvés - Verveur Eau", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  # Ajout de l'annotation pour le nombre total de séquences
...
bar_plot_BarnenezSedi <- ggplot(data.frame(Genus = names(class_counts_BarnenezSedi), Count = as.numeric(class_counts_BarnenezSedi)), 
  ...
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "classs retrouvés - Barnenez Sédiments", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  # Ajout de l'annotation pour le nombre total de séquences
...
bar_plot_VerveurSedi <- ggplot(data.frame(Genus = names(class_counts_VerveurSedi), Count = as.numeric(class_counts_VerveurSedi)), 
  ...
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "classs retrouvés - Verveur Sédiments", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  # Ajout de l'annotation pour le nombre total de séquences
...
bar_plot_BarnenezGDND <- ggplot(data.frame(Genus = names(class_counts_BarnenezGDND), Count = as.numeric(class_counts_BarnenezGDND)), 
  ...
  labs(title = "classs retrouvés - Barnenez GDND", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_VerveurGDND <- ggplot(data.frame(Genus = names(class_counts_VerveurGDND), Count = as.numeric(class_counts_VerveurGDND)), 
  ...
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "classs retrouvés - Verveur GDND", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_BarnenezGDDEP <- ggplot(data.frame(Genus = names(class_counts_BarnenezGDDEP), Count = as.numeric(class_counts_BarnenezGDDEP)), 
  ...
  geom_bar(stat = "identity", fill = "#145C9E") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "classs retrouvés - Barnenez GDDEP", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
bar_plot_VerveurGDDEP <- ggplot(data.frame(Genus = names(class_counts_VerveurGDDEP), Count = as.numeric(class_counts_VerveurGDDEP)), 
  ...
  geom_bar(stat = "identity", fill = "#59C9A5") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, color = "black", size = 3.5) +
  labs(title = "classs retrouvés - Verveur GDDEP", x = "class", y = "Nombre de séquences") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
...
# Extraire les classs uniques de chaque échantillon
classs_BarnenezEau <- unique(data_BarnenezEau$genus)
classs_BarnenezSedi <- unique(data_BarnenezSedi$genus)
classs_BarnenezGDND <- unique(data_BarnenezGDND$genus)
classs_BarnenezGDDEP <- unique(data_BarnenezGDDEP$genus)

# Créer une liste avec les ensembles pour chaque échantillon
sets_listBarnenez <- list(
  BarnenezEau = classs_BarnenezEau,
  BarnenezSedi = classs_BarnenezSedi,
  BarnenezGDND = classs_BarnenezGDND,
  BarnenezGDDEP = classs_BarnenezGDDEP
)
...
venn_plotBarnenez2 <- ggvenn(sets_listBarnenez2, show_elements = TRUE,
  ...
  text_size = 4,  # Ajustez la taille de la police pour les classs
  label_sep = "
  ",
  count_column = NULL,
)
...
```{r}
# Extraire les classs uniques de chaque échantillon
classs_VerveurEau <- unique(data_VerveurEau$genus)
classs_VerveurSedi <- unique(data_VerveurSedi$genus)
classs_VerveurGDND <- unique(data_VerveurGDND$genus)
classs_VerveurGDDEP <- unique(data_VerveurGDDEP$genus)

# Créer une liste avec les ensembles pour chaque échantillon
sets_listVerveur <- list(
  VerveurEau = classs_VerveurEau,
  VerveurSedi = classs_VerveurSedi,
  VerveurGDND = classs_VerveurGDND,
  VerveurGDDEP = classs_VerveurGDDEP
)
...
venn_plotVerveur <- ggvenn(sets_listVerveur , show_elements = TRUE,
  ...
  text_size = 3.5,  # Ajustez la taille de la police pour les classs
  label_sep = "
  ",
  count_column = NULL,
  show_outside = c("auto", "none", "always")
...
grid.arrange(bar_plot_combined2)

grid.arrange(bar_plot_combined)

grid.arrange(bar_plotBarnenezTot)

grid.arrange(bar_plotVerveurTot)


grid.arrange(bar_plotBarnenez, bar_plotVerveur,
             ncol = 2)

grid.arrange(bar_plot_BarnenezEau, bar_plot_BarnenezSedi, bar_plot_BarnenezGDND, bar_plot_BarnenezGDDEP,
...
grid.arrange(venn_plotBarnenez, venn_plotVerveur,
             ncol = 2)
...
ggsave("Figures/FiguresBarnenez/venn_plotBarnenez.png", venn_plotBarnenez, width = 6, height = 6, units = "in")
...
ggsave("Figures/FiguresVerveur/bar_plotVerveurTot.png", bar_plotVerveurTot, width = 6, height = 6, units = "in")
```
